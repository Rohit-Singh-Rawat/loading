...
`pnpm test` runs the Vitest suite in `packages/loading/tests`. It renders every spinner in `SPINNERS` to static markup and checks the motion contract, so it needs no browser.
...
1. Create `packages/loading/src/<name>.tsx` following the conventions above; export it from `src/index.ts` and add it to `SPINNERS` in `src/spinners.ts`, the registry keyed by `ld-` key that the showcase renders from and the tests and the consumer check iterate.
2. Add its default duration to `SPINNER_MOTION` in `packages/loading/src/motion.ts`. The key is the spinner's `ld-` key, and `SpinnerStyle`/`spinnerRoot` will not type-check without it; `SPINNERS` will not type-check until every key has a component.
3. Add a row to `packages/loading/README.md`.
4. Register it in `apps/web/src/components/spinners/index.ts` (`CATALOG`, exported as `SPINNER_ITEMS`). This drives the homepage, the sidebar, previous/next, and `generateStaticParams` — **array position is the display order**. `slug` is typed `SpinnerName`, so it must be the spinner's `ld-` key; preview customization reads the default speed from `SPINNER_MOTION` under that key, and the entry only supplies the slider's `max`/`min`. Rendering components look the spinner up in the library's `SPINNERS` under that slug, and the catalog checks options against that component through a type-only import. A spinner with a prop of its own lists it under `options` — label, prop name, and the values in the order the control shows them, first value the library default, enforced as a nonempty tuple — and `CustomizePanel` renders a segmented control for each.
...
6. Write `apps/web/src/content/snippets/<slug>.mdx` — the opening code example above the prose, a single fenced block titled `<slug>.tsx`. It is static: the size control drives the preview, not this. `/spinners/<slug>/markdown` puts it above the prose too.
7. Add the demo files those `<Demo />` tags name at `apps/web/src/content/demos/<slug>/<demo>.tsx` — `<Demo name="<demo>" />` names the demo without its directory, and `page.tsx` binds the slug that resolves the rest. The `.tsx` is the default-exported component that renders and the single source of the code shown. Beside it goes a one-line `.mdx`, ```` ```tsx title="<slug>.tsx" file=./<demo>.tsx ````, which `remark-code-import` fills from the `.tsx` at build time; `/spinners/<slug>/markdown` reads the same `.tsx`. Match the set the other spinners already have. Either file missing is a build error.
