...
5. Write no prose at all. Every spinner documents the same five sections — Size, Custom Classes, Color, Duration, State — and they live once in `apps/web/src/content/spinners/_shared.mdx`, rendered by `page.tsx` and read by `lib/spinner-markdown.ts`, so a heading added there reaches the prose, the TOC, and `/spinners/<slug>/markdown` for every spinner at once. There is no per-spinner prose file; what makes a spinner distinct belongs in its `description` in `CATALOG` and in the demos.
