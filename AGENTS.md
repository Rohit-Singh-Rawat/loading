...
5. Write no shared prose. Every spinner documents the same five sections — Size, Custom Classes, Color, Duration, State — and they live once in `apps/web/src/content/spinners/_shared.mdx`, rendered by `page.tsx` and read by `lib/spinner-markdown.ts`, so a heading added there reaches the prose, the TOC, and `/spinners/<slug>/markdown` for every spinner at once. What makes a spinner distinct belongs in its `description` in `CATALOG` and in the demos. Only a spinner with a prop of its own gets a prose file, `apps/web/src/content/spinners/<slug>.mdx`, holding one `##` section per prop with its own `<Demo />`; it renders after the shared sections and reaches the TOC and the markdown route the same way. The site imports it whenever the catalog entry has `options`, so an entry with options and no prose file is a build error.
