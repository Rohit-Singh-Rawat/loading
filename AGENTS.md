...
- Every code sample on the site is a `CodeBlock` fed token lines from `src/lib/code.ts`, which knows the one shape a sample takes — an import and a component with props, alone or in a row — and colours each token kind through the six `--code-*` properties in `globals.css`. There is no highlighter and no fenced code in `.mdx`; MDX carries prose and `<Demo />` tags only. `~/Developer/jakub.kr` and `~/Developer/interfaces` share the MDX and dropdown patterns; check those repos before adding web UI here.
