...
5. Write no prose at all in the common case. The five sections every spinner shares — Size, Custom Classes, Color, Duration, State — live once in `apps/web/src/content/spinners/_shared.mdx`, rendered by `page.tsx` and read by `lib/spinner-markdown.ts`, so a heading added there reaches the prose, the TOC, and `/spinners/<slug>/markdown` for every spinner at once. Add `apps/web/src/content/spinners/<slug>.mdx` **only** when a spinner needs a section of its own; it is appended after the shared ones, and unlike the other content files it is optional — both readers fall back when it is absent.
...
7. Add the demo files those `<Demo />` tags name, as a **pair** at `apps/web/src/content/demos/<slug>/<demo>.{tsx,mdx}` — `<Demo name="<demo>" slug={props.slug} />` names the demo without its directory, and the slug resolves the rest — the `.tsx` is the default-exported component that renders, the `.mdx` is one fenced block titled `<slug>.tsx` holding the same code, which is what the reader sees and what `/spinners/<slug>/markdown` splices in. Match the set the other spinners already have. The pair is written by hand and nothing checks the two agree, so change them together. Either file missing is a build error.
