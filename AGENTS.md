...
6. Write `apps/web/src/content/snippets/<slug>.mdx` — the opening code example above the prose, a single fenced block titled `<slug>.tsx`. On the page it is live: the customization controls rewrite the JSX with the props they set, and the live snippet borrows its six token colours from this block's build-time highlighting, so keep the shape the other snippets have — the import and `<X size={48} />`, nothing else — or the page fails to build. The file as written is the default state and what `/spinners/<slug>/markdown` puts above the prose.
