...
4. Register it in `apps/web/src/components/spinners/index.ts` (`CATALOG`, exported as `SPINNER_ITEMS`). This drives the homepage, the sidebar, previous/next, and `generateStaticParams` — **array position is the display order**. `slug` is typed `SpinnerName`, so it must be the spinner's `ld-` key; preview customization reads the default speed from `SPINNER_MOTION` under that key, and the entry only supplies the slider's `max`/`min`. Register its component under the same slug in `apps/web/src/components/spinners/components.ts`. Only rendering components import that registry; the catalog imports its type to check options against the registered component. A spinner with a prop of its own lists it under `options` — label, prop name, and the values in the order the control shows them, first value the library default, enforced as a nonempty tuple — and `CustomizePanel` renders a segmented control for each.
